...
enum class kV0 { kNegative,
                kPositive,
                kNDaughters };
enum class AntiV0 { kPositive,
                    kNegative,
                    kNDaughters };
enum class Cascade { kBachelor,
                     kV0,
                     kNegative,
                     kPositive,
                     kNDaughters };
enum class AntiCascade { kBachelor,
                         kV0,
                         kPositive,
                         kNegative,
                         kNDaughters };
...
struct OnTheFlyDecayer {
  ...
  Configurable<float> maxRapidity{"maxRapidity", 2.5, "Only decay particles that appear within selected eta range"};
  ...
  bool checkDecayChannel(const int pdgMother, const std::vector<o2::upgrade::OTFParticle>& daus)
  {
    switch (pdgMother) {
      case PDG_t::kK0Short:
        if (daus.size() != static_cast<size_t>(kV0::kNDaughters)) {
          return false;
        }
        return (daus[static_cast<int>(kV0::kNegative)].pdgCode() == PDG_t::kPiMinus &&
                daus[static_cast<int>(kV0::kPositive)].pdgCode() == PDG_t::kPiPlus);
      case PDG_t::kLambda0:
        if (daus.size() != static_cast<size_t>(kV0::kNDaughters)) {
          return false;
        }
        return (daus[static_cast<int>(kV0::kNegative)].pdgCode() == PDG_t::kPiMinus &&
                daus[static_cast<int>(kV0::kPositive)].pdgCode() == PDG_t::kProton);
      case PDG_t::kLambda0Bar:
        if (daus.size() != static_cast<size_t>(AntiV0::kNDaughters)) {
          return false;
        }
        return (daus[static_cast<int>(AntiV0::kNegative)].pdgCode() == PDG_t::kProtonBar &&
                daus[static_cast<int>(AntiV0::kPositive)].pdgCode() == PDG_t::kPiPlus);
      case PDG_t::kXiMinus:
        if (daus.size() != static_cast<size_t>(Cascade::kNDaughters)) {
          return false;
        }
        return (daus[static_cast<int>(Cascade::kBachelor)].pdgCode() == PDG_t::kPiMinus &&
                daus[static_cast<int>(Cascade::kV0)].pdgCode() == PDG_t::kLambda0 &&
                daus[static_cast<int>(Cascade::kNegative)].pdgCode() == PDG_t::kPiMinus &&
                daus[static_cast<int>(Cascade::kPositive)].pdgCode() == PDG_t::kProton);
      case PDG_t::kXiPlusBar:
        if (daus.size() != static_cast<size_t>(AntiCascade::kNDaughters)) {
          return false;
        }
        return (daus[static_cast<int>(AntiCascade::kBachelor)].pdgCode() == PDG_t::kPiPlus &&
                daus[static_cast<int>(AntiCascade::kV0)].pdgCode() == PDG_t::kLambda0Bar &&
                daus[static_cast<int>(AntiCascade::kPositive)].pdgCode() == PDG_t::kPiPlus &&
                daus[static_cast<int>(AntiCascade::kNegative)].pdgCode() == PDG_t::kProtonBar);
      case PDG_t::kOmegaMinus:
        if (daus.size() != static_cast<size_t>(Cascade::kNDaughters)) {
          return false;
        }
        return (daus[static_cast<int>(Cascade::kBachelor)].pdgCode() == PDG_t::kKMinus &&
                daus[static_cast<int>(Cascade::kV0)].pdgCode() == PDG_t::kLambda0 &&
                daus[static_cast<int>(Cascade::kNegative)].pdgCode() == PDG_t::kPiMinus &&
                daus[static_cast<int>(Cascade::kPositive)].pdgCode() == PDG_t::kProton);
      case PDG_t::kOmegaPlusBar:
        ...
        return (daus[static_cast<int>(AntiCascade::kBachelor)].pdgCode() == PDG_t::kKPlus &&
                daus[static_cast<int>(AntiCascade::kV0)].pdgCode() == PDG_t::kLambda0Bar &&
                daus[static_cast<int>(AntiCascade::kPositive)].pdgCode() == PDG_t::kPiPlus &&
                daus[static_cast<int>(AntiCascade::kNegative)].pdgCode() == PDG_t::kProtonBar);
      default:
  ...
  void process(aod::McCollision const&, aod::McParticles const& mcParticles)
  {
    ...
    for (int index{0}; index < static_cast<int>(mcParticles.size()); ++index) {
      ...
      allParticles.push_back(o2::upgrade::OTFParticle{particle});
    }
    ...
    for (int index{0}; index < static_cast<int>(allParticles.size()); ++index) {
      const auto& particle = allParticles[index];
      ...
      tableMcParticlesWithDau(particle.collisionId(), particle.pdgCode(), particle.statusCode(),
                              particle.flags(), particle.getMotherSpan(), particle.getDaughters().data(), particle.weight(),
